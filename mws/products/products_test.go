...
func TestProducts_GetServiceStatus(t *testing.T) {
	config := mws.Config{
		SellerId:  "xx",
		AuthToken: "xx",
		Region:    "DE",
...
func TestProducts_GetProductCategoriesForASIN(t *testing.T) {
	config := mws.Config{
		SellerId:  "xx",
		AuthToken: "xx",
		Region:    "DE",
	...
	// Example 1
	fmt.Println("------GetServiceStatus------")
	statusResponse, err := productsClient.GetMatchingProduct([]string{})
	// Check http client error.
	if err != nil {
		fmt.Println(err.Error())
	}
